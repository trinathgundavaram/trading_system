...
HARD_VETOES_CATALOG = {
    ...
    "vetoes": [
        ...
        {"code": "DAILY_LOSS (REMOVED §54, Phase 2.5)", "description": "Was: config.yaml's risk.daily_loss_limit_triggered is on. Removed because NO CODE PATH EVER SET THAT FLAG - it could only fire from a hand-edited config while this catalogue advertised it as a live veto. No capability lost: the real daily-loss control is rules/risk_rules.py's RiskEngine.check() + trip_kill_switch_if_needed(), which use §8's equity-scaled limit (the tighter of max_daily_loss_usd and max_daily_loss_pct of equity) and route a breach through KILL_SWITCH above. engine/position_management.py also read the flag as a PRIORITY-1 exit-everything trigger, so the removal additionally closes a path where hand-setting a writerless key liquidated the book."},
        {"code": "PROFIT_LOCK (REMOVED §54, Phase 2.5)", "description": "Was: config.yaml's risk.daily_profit_lock_triggered is on. Removed for the same reason as DAILY_LOSS - no writer, hand-edit only. There is currently no daily profit-lock control; if one is wanted it should be built with a writer and a test rather than reinstated as a config key."},
...
ACCOUNT_RISK_CATALOG = {
    "engine": "Per-trade / per-day ACCOUNT-level checks, distinct from market_filters.py "
              "(market-wide) and buy/sell_rules.py (per-ticker signal rules). rules/risk_rules.py "
              "owns the day/drawdown budget (RiskEngine.check + trip_kill_switch_if_needed); the "
              "count and size ceilings are enforced in the trader that is about to place the order, "
              "which is where the position actually gets created. Each entry below names its own "
              "enforcement site - §54 (Phase 2.5) found this section attributing all eight to "
              "rules/risk_rules.py, where a dead parallel copy of three of them had been sitting "
              "unused and slowly diverging from the live definitions.",
    "checks": [
        {"name": "kill_switch", "enforced_in": "rules/risk_rules.py RiskEngine.check + rules/hard_vetoes.py", "description": "config.yaml's risk.kill_switch_triggered - manual or auto-tripped (see trip_kill_switch_if_needed below), halts all trading until manually cleared + restarted"},
        {"name": "max_trades_per_day", "enforced_in": "rules/risk_rules.py RiskEngine.check", "description": "config.yaml's risk.max_trades_per_day - shared budget across DAY+SWING+HYBRID combined, counted per book (§7): a paper sell consumes it exactly as a paper buy does"},
        {"name": "max_daily_loss", "enforced_in": "rules/risk_rules.py RiskEngine.check + trip_kill_switch_if_needed", "description": "Realized P&L today must stay above the §8 limit, which is the TIGHTER of risk.max_daily_loss_usd and risk.max_daily_loss_pct of the book's equity at cost - NOT the raw dollar cap. Auto-trips the kill switch rather than just blocking new entries once breached"},
        {"name": "max_intraday_drawdown", "enforced_in": "rules/risk_rules.py drawdown_breach", "description": "§11. Today's worst peak-to-trough on the equity curve must stay under risk.max_intraday_drawdown_pct. Blocks new entries for the rest of the day. Sees what realized P&L cannot: an account can round-trip 4% intraday and finish flat, having taken every bit of that risk. Measured per book from daily_stats.paper_max_drawdown, written by storage/database.update_drawdown() on every equity point"},
        {"name": "max_running_drawdown", "enforced_in": "rules/risk_rules.py drawdown_breach + trip_kill_switch_if_needed", "description": "§11. Distance from the all-time equity high must stay under risk.max_running_drawdown_pct. Blocks entries entirely - this is not a bad day, it is the strategy having stopped working, and clearing it is a human act. Unlike the intraday figure this one is a current distance, not a high-water mark, so a genuine recovery does clear it"},
        {"name": "max_positions", "enforced_in": "engine/paper_trader.py + engine/live_trader.py (advisory), storage/database.try_open_position (authoritative, §14)", "description": "config.yaml's trading.max_positions - open position count ceiling, separate from the DAY-specific trading.max_day_positions cap (see Trading Modes above). The pre-check in the trader decides strategy and produces the log line; the cap is actually held inside try_open_position's transaction, because six parallel workers can all read the same count"},
        {"name": "max_position_size", "enforced_in": "engine/position_sizing.py + engine/live_trader.py", "description": "Any single position's dollar amount is clamped to risk.max_position_size_usd - a clamp, not a veto: an oversized suggestion is reduced to the ceiling rather than refused"},
        {"name": "sufficient_buying_power", "enforced_in": "engine/paper_trader.py (paper purse); the broker (real)", "description": "A paper buy is skipped when the purse holds less than the suggested amount, exactly as a real account would refuse. For the real book this codebase does not place orders from Python, so buying power is the broker's to enforce"},
    ],
...
PORTFOLIO_RISK_CATALOG = {
    ...
    "dimensions": [
        ...
        {"name": "Simultaneous high-volatility positions", "cap": "high_vol_atr_pct_threshold 5.0% of price (PROVISIONAL), max_simultaneous_high_vol_positions 4 (PROVISIONAL)", "description": "Counts open positions whose ATR is already >=5% of price. A high-vol candidate hitting the cap -> 0.0; one below the cap -> 0.5 (reduced size, near the limit). PROVISIONAL (2026-07-26): unlike the drawdown caps, 5.0 was never measured - it is a round number, not a percentile of any observed ATR distribution. It cannot be fitted while engine/portfolio_risk.py's high_vol_proxy_count is non-zero, because the open-position side is then part real ATR and part stop-distance proxy (§C3) and a percentile over a mixed population describes neither. Recalibrate once that count has been ~zero for ~20 sessions, and record the measured value with its percentile and sample size in config.yaml - see the comment there."},
    ],
